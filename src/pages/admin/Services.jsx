...
export default function Services() {
    ...
    const loadServices = async () => {
        try {
            setLoading(true);
            setError("");
            setServices(await dataAPI.fetchServices(true));
        } catch (err) {
    ...
    const handleDeleteClick = async (id) => {
        if (window.confirm("Apakah Anda yakin ingin menghapus layanan ini dari katalog?")) {
            await dataAPI.deleteService(id);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        await dataAPI.saveService(form, isEditing ? currentId : null);
        await loadServices();
    ...
    return (
        <div className="w-full min-h-screen bg-[#080807] text-[#D3CDC3] font-sans antialiased pb-12">
            <Container>
                ...
                {loading && <div className="mb-4 text-sm text-white/40">Memuat layanan...</div>}
